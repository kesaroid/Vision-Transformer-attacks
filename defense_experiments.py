...
def test(model, defend=False, output='Results', max_perturb=6.0):
    ...
    for i, (data, target) in enumerate(test_loader):  # tqdm(test_loader)
        ...
        if defend:
            # downsize adversary
            pert_16x16 = torch.nn.functional.interpolate(pert_image, size=(16, 16), mode='bilinear',
                                                         align_corners=False)
...
if __name__ == "__main__":
    ...
    print('defense experiments')

    max_perturbations = [i for i in range(3, 10, 2)]
    ks = [i for i in range(1, 8)]

    for max_perturbation in max_perturbations:
        TI = False
        print('No TI, magnitude {}'.format(max_perturbation))
        mifgsm_params = {'eps': max_perturbation, 'steps': iterations, 'decay': 1.0, 'alpha': alpha, 'TI': TI, 'k': 0}

        acc, _, norms = test(model, defend, None, max_perturbation)
        print('--------- Defense fool rate {} ---------'.format(acc))
        print()

        for k in ks:
            TI = True

            print('using TI, magnitude {}'.format(max_perturbation))
            mifgsm_params = {'eps': max_perturbation, 'steps': iterations, 'decay': 1.0, 'alpha': alpha, 'TI': TI, 'k': k}
