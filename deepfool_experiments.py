    ...
    for i, (data, target) in enumerate(test_loader):  # tqdm(test_loader)
        ...
        if attack == 'sta':
            ...
            r, loop_i, label_orig, label_pert, pert_image = deepfool(data.clone().detach().requires_grad_(True),
                                                                     model,
                                                                     max_iter=deepfool_params['iterations'],
                                                                     max_perturb=deepfool_params['max_perturb'])
        elif attack == 'pgd':
...
if __name__ == "__main__":
    ...
    iterations = [i for i in range(1, 20)]
    ...
    for defend in defenses:
        for max_perturbation in max_perturbations:
            for iteration in iterations:
                        ...
                        print('<----------- deepfool with defend={} epsilon={} iterations={} ----------->'.
                              format(defend, max_perturbation, iteration), flush=True)

                        deepfool_params = {'max_perturb': max_perturbation,
